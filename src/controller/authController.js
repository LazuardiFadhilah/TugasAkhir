...
class AuthController {
  ...
  login = async (req, res) => {
    try {
      ...
      // generate Token
      const { accessToken, expiresIn, refreshToken } = await this.generateToken(
        user
      );
  ...
  register = async (req, res) => {
    try {
      ...
      if (!isRoleValid) {
        return res
          .status(400)
          .json({ message: "Role must be Creator or Super Admin" });
      }
    ...
    //  confirm email
      SendEmailController.sendEmailRegister(user.email, user.fullName);
